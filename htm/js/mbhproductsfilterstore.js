...
var StoreProductsFilter$=(function(){
  ...
  var bExibeCats=false;  //indica se exibe categorias 
  var bExibeAdics=false;  //indica se exibe adicionais (descritores especiais)
  ...
  function fnMain(){
    ...
    if(typeof(FCLib$.pQuery)=="object" && FCLib$.pQuery!=null && oFilOut){
      ...
      if(sFiltrandoPor!="" || sOutrosFiltros!="" || sFiltroTxt!=""){
        ...
        sFiltrandoPor+="<li><div class='FilRemoveFilters'><a href='/listaprodutos.asp?idloja="+FC$.IDLoja+"'>"+ aXYZ[2][IdiomaLoja] +"</a></div></li>";
        sFiltrandoPor+="<li><div class='FilMoreOptionsSearch'><a href='/buscaavancada.asp?idloja="+FC$.IDLoja+sParamsBusca+"'>"+ aXYZ[3][IdiomaLoja] +"</a></div></li>";
        sFiltrandoPor+="</ul>";
      }
  ...
  //se passar iAdic,idItemAdic em iAdic contém o número do adicional (1,2 ou 3) e em idItemAdic o id para filtrar. Neste caso monta a URL com o informado
    ...
    if(idcat>0){
      if(urlcat!=""){
        sURL="/"+ catPath +"/"+urlcat +"?idloja="+ FC$.IDLoja;
        if(sSearchFil!="")sParams+="&fil="+ sSearchFil;
      }
      else{
        sURL="/listaprodutos.asp?idloja="+ FC$.IDLoja +"&idcategoria="+ idcat;
        if(sSearchFil!="")sParams+="&fil="+ sSearchFil;
      }
    }
    else{  
      if(cat.url!=""){
        sURL="/"+ catPath +"/"+cat.url +"?idloja="+ FC$.IDLoja;
      }
      else{
        sURL="/listaprodutos.asp?idloja="+ FC$.IDLoja;
        if(typeof(cat)=="object" && cat.id>0)sParams+="&idcategoria="+ cat.id;
      }
      if(IDsFilters!="")sParams+="&fil="+ IDsFilters;
    }   
    if(idProd>0)sParams+="&idproduto="+ idProd;
  ...
  function fnRemoveFilter(iTipo,IsFiltro){
    ...
    if(iTipo==0 || cat.url==""){  //se estã removendo categoria ou não tem url da categoria
      sURL="/listaprodutos.asp?idloja="+ FC$.IDLoja;
